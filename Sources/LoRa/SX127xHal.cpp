...
void SX127xHal::init()
{
  ...
  lora_rx_switch(0);
  lora_tx_switch(0);
  lora_spi_set_frequency(10000000);
  lora_csn_on();
  delay(100);
  lora_reset_switch(0);
  delay(100);
...
void ICACHE_RAM_ATTR SX127xHal::readRegisterBurst(uint8_t reg, uint8_t numBytes, uint8_t *inBytes)
{
  ...
  lora_csn_off();
  SPI_RADIO_SendBlock(buf, 1);
  SPI_RADIO_ReceiveBlock(buf+1, numBytes);
  lora_csn_on();
...
uint8_t ICACHE_RAM_ATTR SX127xHal::readRegister(uint8_t reg)
{
  ...
  lora_csn_off();
  SPI_RADIO_SendBlock(buf, 1);
  SPI_RADIO_ReceiveBlock(buf+1, 1);
  lora_csn_on();
#else
  digitalWrite(GPIO_PIN_NSS, LOW);
  SPI.transfer(buf, 2);
...
#ifdef ERFLY_BOARDI6
  lora_csn_off();
  SPI_RADIO_SendBlock(buf, numBytes + 1);
  lora_csn_on();
#else
