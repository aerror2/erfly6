...
uint16_t evalChkSum(void) {
  uint16_t sum = 0;
  uint8_t *p;
  p = (uint8_t *)&g_eeGeneral.calibMid;
...
void eeGeneralDefault() {
  ...
  g_eeGeneral.vBatWarn = 45;
  g_eeGeneral.stickMode = 1;
  g_eeGeneral.beeperVal = 1;
  g_eeGeneral.speakerMode = 1;
  g_eeGeneral.speakerPitch = 20;
  g_eeGeneral.lightSw = MAX_DRSWITCH; // ON
  g_eeGeneral.templateSetup = 21;
...
void modelDefaultWrite(uint8_t id) {
  ...
  g_model.switchWarningStates = 0x48;
#ifdef V2
...
bool eeDuplicateModel(uint8_t id) {
  ...
  for (i = id + 1; i < MAX_MODELS; i++) {
    if (!EFile::exists(FILE_MODEL(i)))
      break;
  }
  if (i == MAX_MODELS)
    return false; // no free space in directory left
...
void eeWriteGeneral() {
  ...
  uint16_t sz = theFile.writeRlc(FILE_GENERAL, FILE_TYP_GENERAL, (uint8_t *)&g_eeGeneral, sizeof(g_eeGeneral), 255);
  if (sz != sizeof(g_eeGeneral))
    alert(PSTR(STR_GENWR_ERROR));
...
void eeCheck(bool immediately) {
  ...
  if (!immediately) {
    if ((get_tmr10ms() - s_eeDirtyTime10ms) < WRITE_DELAY_10MS)
      return;
    if (s_eeLongTimer) {
      if (--s_eeLongTimer) {
        s_eeDirtyTime10ms = get_tmr10ms();
  ...
  if (msk & EE_GENERAL) {
    ...
    if (theWriteFile.writeRlc(FILE_TMP, FILE_TYP_GENERAL, (uint8_t *)&g_eeGeneral,
            sizeof(g_eeGeneral), 200) == sizeof(g_eeGeneral)) {
    ...
    if (theWriteFile.writeRlc(FILE_TMP, FILE_TYP_MODEL, (uint8_t *)&g_model,
            sizeof(g_model), 200) == sizeof(g_model)) 
    {
