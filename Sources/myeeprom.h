...
//    uint8_t   reserved_spare;        //used to be MDVERS - now depreciated
			...
			uint8_t IS_BUS:1;          // I-BUS  or S-BUS out subprotocol
			...
			SafetySwData safetySw[NUM_CHNOUT];
                        /*----- AFHDS2A telemetry (Kotello)----*/
                         //FrSkyData frsky; //11 bytes
                        int8_t IntVoffs;
                        int8_t ExtVoffs;
                        int8_t FuelSource;
                        int16_t FuelMin;
...
/*			int8_t sparepxxFailsafe[2];	// Currently unused*/
			int16_t FreqOffset;	        // Fine tune A7105 LO base frequency
			uint8_t telemetryProtocol;
			int8_t  Failsafe[8];            // Currently unused
			uint8_t failsafeMode:3;
			uint8_t failsafeRepeat:1;
			...
			int8_t XFailsafe[6];		// Currently unused
			uint16_t ServoFreq;         // Servo Frequency
			int8_t rxID[4];
